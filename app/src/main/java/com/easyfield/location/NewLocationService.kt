...
class NewLocationService : LifecycleService() {
    ...
    private val networkCallback: ConnectivityManager.NetworkCallback =
        object : ConnectivityManager.NetworkCallback() {
            override fun onAvailable(network: Network) {
                ...
                val db = AppDatabase.getInstance(this@NewLocationService)
        ...
        override fun onLocationResult(locationResult: LocationResult) {
            ...
            if (locationList.isNotEmpty()) {
                ...
                lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    val db = AppDatabase.getInstance(this@NewLocationService)
                    val list = db.locationDao().getAllLocations()
                    Log.w("sendLocation","size"+list.size)
                    if(list.size>10){
                        sendBulkLocationData(list)
                    }
                }
...
//            .build()
//
//        WorkManager.getInstance(this@NewLocationService).enqueueUniquePeriodicWork(
//            "location_restart",
...
//        )
        ...
        val powerManager = getSystemService(Context.POWER_SERVICE) as PowerManager
         wakeLock = powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "AxisSeed::LocationWakeLock")
...
//                }
    ...
    private fun createNotificationChanel() {
        ...
        val notification: Notification = notificationBuilder.setOngoing(true)
            .setContentTitle("Kirtiman Location")
