...
suspend fun File.compress(
    ...
    val result = decodeSampledBitmapFromFile(this@compress, width, height).run {
        determineImageRotation(this@compress, this).run {
            overWrite(this@compress, this, format, quality)
        }
    }
    return@withContext copyToCache(context, result)
}
